...
# Enforce TLS 1.2 — un-patched Windows 10 PS 5.1 may default to 1.0/1.1,
# which GitHub no longer accepts. -bor preserves any newer protocols already enabled.
...
function Get-Arch {
    ...
    # Fallback: env vars. Under WOW64 (32-bit PS on 64-bit Windows),
    # PROCESSOR_ARCHITECTURE reports "x86"; PROCESSOR_ARCHITEW6432 has the real arch.
...
try {
    ...
    Write-Host ""
    Write-Host "  (Open a new terminal to pick up the updated PATH in future sessions.)" -ForegroundColor DarkGray
}
finally {
